...
This approach aims to use the microstructure model of Lyons and Evans at the macro level.  The motivation behind the flow is not identified as the attention is on the effect on the real exchange rate.  While this may not be totally satisfying, it does allow the effect of international capital flows to be examined.  The most important issue is to identify that part of the international portfolio flow that is forcing change from that facilitating adjustment. In the terminology of Lyons and Evans, it is to distinguish the initiator of the transaction from that of the passive market-maker.  While these aggregate figures do not identify indivudla transactions of economic entities in teh same say, they are highligting a similar economic force for change and it is this force for change that is deemeed to be the factor that will cause the exchange rate to make an adjustment in price.    
\subsection{Microstructure}
 For example, \citep{Evans2002Order} and  \citep{Lyons1995Microstructure} give an overview of the microstructure approach to exchange rates.  The approach combines microstucure techniques that have been developed in the area of equity markets \citep{maureen1998market} with the unique features of the dealer-orientated foreign exchange market to use order flow to enhance exchange rate models.   Lyons and Evans use a unique dataset that records deals undertaken by foreign exchange market-markers (or dealers).  The data show active or \emph{signed} deals as well as passive deals. Signed deals are the ones that initiate the transaction.  It is the party that makes the call that ends in a transaction. Information about signed foreign exchange orders are added to regular macroeconomic variables to create the following model

...
One of the aims of this paper is to understand more about the deviations in the exchange rate from fundamental or intrinsic value.  There are different views about fundamental value.  However, here it will be assumed that Purchasing Power Parity (PPP) determines the fundamental value.  
...
where $z$ is the real exchange rate. Given two countries, an appreciation of the real exchange rate for the home country (a decrease in Z) means that its relative money prices have risen compared to the overseas country.  The theory of PPP says that the real exchange rate should be stable or that is should return to a long-run level. There is ample evidence that the real exchange rate is not stable.  For example, the \emph{Penn Effects} is the tendency of the real exchange rate to increase with the level of economic development (reference).  The interest here is how far capital flows can influence the level of the real exchange rate. 

...
Therefore, excess net demand for overseas equities will cause an appreciation of the overseas currency as a means of inducing risk-averse market-makers and speculators to hold more domestic currency.  Speculators facilitate the decisions of equity investors through price concession. This is consistent with the Keynes-Hicks model of speculations.  See \citet{KeynesHedge} and \citet{HicksHedge}.   
...
It is assumed that exogenous capital flows ($z_t$) follow an AR process with a parameter $\Phi$.  This exogenous variation in capital flow is related to the changing attitudes towards risk aversion.  As risk aversion rises, capital tends to return home, as it falls capital seeks overseas destinations.  As such, this exogenous influence could also be related to speculative sentiment.  As shown in \citet{Hayward2013}, the carry-trade is more likely to be successful when conditions are calm and risk appetite high, carry-trades will be unwound during periods of crisis and during high risk episodes. 
...
This is downward sloping as demand for US dollars decreases with the strength of the dollar (decline in $e_0$). $\Gamma$ controls the ability of financiers to take risk. The higher $\Gamma$ the lower the risk-baring and the steeper the demand curve. There are two extremes: when $\Gamma = 0$ financiers are able to bare any level of risk and UIP holds and trade imbalances are accommodated without any need for a change in the value of the exchange rate; as $\Gamma \rightarrow \infty$  financiers are unable to take any risk, there are no financial account transactions and the exchange rate is determined by only by trade imbalances. 

However, in the cases between, for the market to clear in the basic two period model, 
...
For simplicity $\beta + \beta^* = 1$ and therefore, $R + R^* = 1$.  
...
The FDI data come from the US balance of payments, net portfolio flows are from the US Treasury International Capital data, speculative momentum is based on CFTC speculative positions and the interest rate differential is a proxy for the carry-trade. Fuller details of the variables, their source and data preparation are in the Appendix \ref{app1}.   

The preferred model, according to the range of information and diagnostics is the one that does not include the current account, has four lags and uses the three dummy variables as well as the S1 and SPREAD2 measures of sentiment and interest rates.  A model with a linear trend and a constant is the one that best fit the data.  The summary information is presented in Appendix \ref{app2}.  It reveals that the system is stable as the roots of the autoregressive coefficients are all within the unit circle.    

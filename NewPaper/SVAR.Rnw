...
\citet{HauEquity} only assess equity market flows with bond supplies in home and overseas assumed to be perfectly price elastic.  Interest rates are equal and exogenous; the supply of equity is fixed while the supply of foreign exchange is less than perfectly elastic.  This paper proposes the addition of less than elastic supply of bonds, money market instruments and real investment opportunities or expectations that the value of these assets and instruments can change.  The additions means that demand for foreign direct investment, bonds and speculative activity can now affect the foreign exchange market 

Note 20 closes the model.  The flow is offset by a movement of currency that is absorbed by the central bank.  

In this case, it is used to explain deviations from purchasing power partiy. The main difference in the module used here form the one that has been presented by \citet{HauEquity} is that speculative flows and foreign direct investment are included and the idea that international assets are in elastic supplty is not just restricted to the bond market. 

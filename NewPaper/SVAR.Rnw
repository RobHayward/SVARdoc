...
Identifying speculative flows has been a challenge due to the scarcity of data.  A standard approach has been to use \citet{BISbanking}. For example, \citet{Bruno2014} model the global liquidity cycle of internationa banks; \citet{AdamsKaneGlobal} try to assess the relative importance or willingness to lend, the level of liqudity and measures of solvency in determing capital flow.  However, it is difficult within this data to distinguish between bank lending for real business projects and that for speculative activity.  A bank loan that originates from overseas is much more likely to be beneficial for the recipient country than one that is merely financing an exchange rate speculation. Howev, \citet{Cerutti2014} combine BIS with proprietory data to analyse the flow between international and local banks. 
...
\section{The model:literature}\label{secref:2}
...
A standard linear empirical exchange rate model presented by \citet{Siourounis2004Capital}, he finds that net US equity purchase has a stable and consistent impact on UK, German and Swiss exchange rates, but that net purchase of US or overseas bonds have no effect.   A vector auto-regression (VAR) is employed and impulse response functions (IRF) and variance decomposition are estimated to help understand the relative improtance of difference types of capital flow.  
...
\section{The model: methodological issues}\label{secref:3}

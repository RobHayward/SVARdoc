...
The standard models of exchange rates and capital flows concentrate on the current account, intert-temporal substitution and the savings across countries. \citet{OandR} provide an overview of the standard open-economy macro model that concentrates on relative prices and the adjustment via the current account.  International financial flows can be incorporated so that adustment to exogenous innovations like productivity shocks through inter-temporal adjustments.  See...

The increasd availabiliuty of data has allowed more attention to be devoted to capital flows and their effects as well as details like the relationship between gross and net flows (example) and valuation effects (rey).  
...
This is where the work section begins.  This needs to be modified and updated using the notes in the hard copy. 
\section{The Model}\label{secref:3}

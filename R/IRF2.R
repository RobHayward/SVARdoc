# Create impulse response with VAR1---------------------
head(da)
# adjust spread data-------------------------
set.seed(123)
...
# System 2 CNB---------------------------------------
irf.rtwicnb <- irf(Var1, n.ahead=8,impulse='CNB', response='RTWI',ortho=TRUE, 
...
# System 2 CNE------------
irf.rtwicne <- irf(Var1, n.ahead=8,impulse='CNE', response='RTWI',ortho=TRUE, 
                   cumulative=TRUE,boot=TRUE, runs=100)
a<-irf.rtwicne$irf
b<-irf.rtwicne$Upper
c<-irf.rtwicne$Lower
...
abline(h=0, col='red',lty=6)
# System 2 CNFDI --------------------------------
irf.rtwicnfdi <- irf(Var1, n.ahead=8,impulse='CNFDI', response='RTWI',ortho=TRUE, 
                     cumulative=TRUE,boot=TRUE, runs=100)
a<-irf.rtwicnfdi$irf
...
abline(h=0, col='red',lty=6)
# System 2 COT--------------------------------------
irf.rtwicot <- irf(Var1, n.ahead=8,impulse='COT', response='RTWI',ortho=TRUE, 
                   cumulative=TRUE,boot=TRUE, runs=100)
a<-irf.rtwicot$irf
b<-irf.rtwicot$Upper
c<-irf.rtwicot$Lower
...
abline(h=0, col='red',lty=6)
# System 2 Spread----------------------------
irf.rtwispread <- irf(Var1, n.ahead=8,impulse='SPREAD2', response='RTWI',ortho=TRUE, 
                      cumulative=TRUE,boot=TRUE, runs=100)
